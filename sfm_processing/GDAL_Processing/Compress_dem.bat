...
FOR %%f in (*.tif) DO (
    ...
    REM gdal_translate -of GTIFF -a_nodata none -co COMPRESS=DEFLATE -co PREDICTOR=2 -co TILED=YES !src! !dest!
    REM gdaladdo --config COMPRESS_OVERVIEW JPEG --config PHOTOMETRIC_OVERVIEW YCBCR --config INTERLEAVE_OVERVIEW PIXEL -r average !dest!
    REM gdalinfo -stats -hist !dest!

    REM compress and add overviews/stats to SFM DSM
    gdal_translate -of GTIFF -co COMPRESS=DEFLATE -co PREDICTOR=3 -co TILED=YES !src! !dest!
    gdaladdo --config COMPRESS_OVERVIEW JPEG --config INTERLEAVE_OVERVIEW PIXEL -r average !dest!
